...
DATASET = 'CIFAR10'
PRETRAINED = True
PRETRAINED_PATH = '../Models/Pretrained/CIFAR10_low_pass/ResNet18/model.t7'
...
# Load the data
BANDWIDTH = 16
trainloader, testloader, trainset, testset, _, _, proc_mean, proc_std = get_processed_dataset_loaders(lambda x: dct_low_pass(x, bandwidth=BANDWIDTH), DATASET, DATASET_DIR, BATCH_SIZE)
...
if not PRETRAINED:
    ...
    SAVE_TRAIN_DIR = os.path.join(TREE_ROOT, '../Models/Generated/%s_low_pass/%s/' % (DATASET, model.__class__.__name__))
...
RESULTS_DIR = os.path.join(TREE_ROOT, '../Results/margin_%s_low_pass/%s/' % (DATASET, model.__class__.__name__))
